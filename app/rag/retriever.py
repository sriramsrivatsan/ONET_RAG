...
class HybridRetriever:
    ...
    def __init__(
        ...
        logger.info(
            f"✓ v4.2.0: Generic pattern engine loaded with {len(self.pattern_engine.categories)} task categories",
            show_ui=False
        )
    ...
    def retrieve(
        ...
        if detected_category:
            ...
            logger.info(f"✓ v4.2.0: Detected task category: {detected_category}", show_ui=False)
        ...
        # V4.0.0: GENERIC task category queries (replaces hardcoded document creation)
            ...
            logger.info(f"✓ v4.2.0: Task category query detected ({detected_category}) - using generic pattern matching", show_ui=False)
            ...
            logger.info(f"✓ v4.2.0: Generic pattern matching found {len(matching_df)} rows for {detected_category}", show_ui=False)
        ...
        # Handle breakdown queries (V4.0.0: now uses generic pattern matching)
            ...
            logger.info(f"✓ v4.2.0: Breakdown query for {detected_category}", show_ui=False)
            ...
            logger.info(f"✓ v4.2.0: Breakdown query - generic pattern matching found {len(matching_df)} rows", show_ui=False)
...
"""
    ...
    def _computational_retrieval(
        ...
        if params.get('industry_ranking'):
            ...
            if detected_category:
                ...
                logger.info(f"✓ v4.2.0: Generic pattern matching for {detected_category} on full dataset", show_ui=False)
                ...
                logger.info(f"✓ v4.2.0: Pattern matching found {len(pattern_df)} rows (of {len(self.df)} total)", show_ui=False)
        ...
        if pattern_detected and is_job_query and not is_task_query:
            ...
            if detected_category:
                logger.info(f"✓ v4.2.0: Pattern match FOUND ({detected_category}) - triggering occupation analysis", show_ui=False)
                ...
                logger.info(f"✓ v4.2.0: Pattern analysis complete: {len(occupation_analysis.get('top_occupations', []))} occupations matched", show_ui=False)
                computational_results['occupation_pattern_analysis'] = occupation_analysis
    ...
    def _analyze_occupations_from_matches(
        ...
        analysis = {
            ...
            'method': 'v4.2.0_generic_pattern_engine'
        }
        
        logger.info(f"✓ v4.2.0: Occupation analysis: {len(occupation_scores)} occupations match pattern", show_ui=False)
    ...
    def _analyze_occupations_by_pattern(
        ...
        Kept for backward compatibility but should not be used in v4.2.0+
        ...
        Returns:
            Dictionary with occupation rankings
        """
        logger.warning("⚠️  Using legacy _analyze_occupations_by_pattern (should use _analyze_occupations_from_matches in v4.2.0)", show_ui=False)
